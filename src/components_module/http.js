            ...
            var req = new Promise(function(resolve,reject){
                ...
                xhr.onload = function(){
                    ...
                    if(this.status === 401||this.status === 403){
                        localStorage.setItem("_SHANDONG_PASS_IN_WITHOUT_TOKEN",0)
                        window.location.href ="/#/login"
                        return
                    }else{
                        localStorage.setItem("_SHANDONG_PASS_IN_WITHOUT_TOKEN",1)
                    }
                    if(this.status >= 200 && this.status < 300 || this.status === 304){
                        ...
                        if(value.token){
                            Token = value.token;
                            localStorage.setItem("_SHANDONG_PASS_IN_TOKEN",Token)

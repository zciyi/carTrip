      ...
      this.$request({
          ...
          if(re){
            ...
            for(var i=0;i<random;i++){
              var obj ={}
              obj["_SHANDONG_PASS_ININ"+random]= Date.now().toString(16) + "_" + Math.ceil(Math.random() * 1000)
              dat.push(obj)
              key.push("_SHANDONG_PASS_ININ"+random);
            }
            localStorage.setItem("_SHANDONG_PASS_IN",JSON.stringify({
              _SHANDONG_PASS_IN:key
              ,_SHANDONG_PASS_IN_VALUE:dat
              ,TIME:new Date().getTime()
              ,_SHANDONG_SET_SESSION:re.sessionId
            }) )
            me.tip('登录成功');
